...
const getFeaturedKitchens = catchAsync(async (req: Request, res: Response) => {
  const result = await PublicServices.getFeaturedKitchens()

  sendResponse(res, {
    statusCode: StatusCodes.OK,
    success: true,
    message: 'Featured kitchens retrieved successfully',
    data: result,
  })
})

const getAllKitchens = catchAsync(async (req: Request, res: Response) => {
  const filters = pick(req.query, ['searchTerm', 'city', 'categoryId'])
  const paginationOptions = pick(req.query, paginationFields)

  const result = await PublicServices.getAllKitchens(filters, paginationOptions)
