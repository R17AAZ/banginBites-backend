...
type IFolderName = 'images' | 'media' | 'documents' | 'image' | 'profile'
    ...
    upload(req, res, async error => {
      ...
      try {
        ...
        if (req.files) {
          ...
          // Merge processedFiles with req.body to support appending to existing arrays (like images)
          for (const [key, value] of Object.entries(processedFiles)) {
            if (Array.isArray(req.body[key]) && Array.isArray(value)) {
